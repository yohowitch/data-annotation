...
def generate_match(sentence, match_segment):
    ...
    output_string = r"\colorbox{green!30}{$\underline{\overline{\\text{" +''.join(words)+"}}}$}$^{{\color{blue}{"+entity_type+"}}}_{{\color{red}{"+entity_type+"}}}$"
    return output_string
...
def generate_gold_left(sentence, match_segment):
    ...
    words = sentence[start:end+1]
    output_string = r"\colorbox{blue!30}{$\overline{\\text{" +''.join(words)+"}}$}$^{{\color{blue}{"+entity_type+"}}}"
    return output_string
...
def generate_pred_left(sentence, match_segment):
    ...
    words = sentence[start:end+1]
    output_string = r"\colorbox{red!30}{$\underline{\\text{" +''.join(words)+"}}$}$_{{\color{red}{"+entity_type+"}}}$"
    return output_string
...
def generate_latex(sentence, gold_bound, pred_bound):
    ...
    for idx in range(len(word_chunk)):
        ...
        if color_chunk[idx] == 2:
            output_string += r"\colorbox{green!30}{" + word_chunk[idx] + '}'
        elif color_chunk[idx] == 1:
            output_string += r"\colorbox{blue!30}{" + word_chunk[idx] + '}'
        elif color_chunk[idx] == 0:
            output_string += word_chunk[idx]
        elif color_chunk[idx] == -1:
            output_string += r"\colorbox{red!30}{" + word_chunk[idx] + '}'
...
def write_head(out_file):
    ## 写入文件开头的内容
    out_file.write(r"%%%%%%%%%%%%%%%%%%%%%%% file typeinst.tex %%%%%%%%%%%%%%%%%%%%%%%%%\n")
    ...
    out_file.write(r"\\begin{document}\n")
    out_file.write(r"\\begin{CJK*}{UTF8}{gbsn}\n")
    out_file.write(r"\mainmatter  % start of an individual contribution\n")
    out_file.write(r"\\title{Annotation Comparison Report}\n")
    out_file.write(r"\\author{SUTDNLP Group}\n")
    out_file.write(r"\\institute{Singapore University of Technology and Design}\n")
    
    out_file.write(r"\\maketitle\n\n")
